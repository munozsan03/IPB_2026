...
class Substrate:
    def __init__(self, smiles, job=None):
        ...
        self.ph_corrected_smiles = self.correct_ph(self.smiles, 8.5)

        # DIAGNOSTIC: Check what RDKit calculates for this SMILES
        true_charge = Chem.GetFormalCharge(Chem.MolFromSmiles(self.ph_corrected_smiles))
        print(f"DEBUG -> SMILES: {self.ph_corrected_smiles} | RDKit Formal Charge: {true_charge}")
        ...
        # Diagnostic check for charge verification
        true_charge = Chem.GetFormalCharge(Chem.MolFromSmiles(self.ph_corrected_smiles))
        print(f"SMILES: {self.ph_corrected_smiles} | RDKit Formal Charge: {true_charge}")
    ...
    def _optimize_geometry(self, opt_method):
        ...
        # Explicitly compute the formal charge using RDKit as a single source of truth
        formal_charge = Chem.GetFormalCharge(Chem.MolFromSmiles(self.ph_corrected_smiles))
        
        # Pass the charge explicitly into autode to avoid parser discrepancies
        mol = autode.Molecule(
            smiles=self.ph_corrected_smiles, 
            name=temp_name,
