...
ALIGNMENT_TABLE = os.path.join(CATNIP_ROOT, "data/alignment_scores.csv")
PRECOMPUTED_PREDICTIONS_CSV = os.path.join(CATNIP_ROOT, "research_code/predictions.csv")

RANKER_PATH = os.path.join(
    CATNIP_ROOT, "research_code/for_backend/scoring_formula_50_2.cb"
)

OUTPUT_PREDICTIONS = os.path.join(
    CATNIP_ROOT, "test_catnip/external_substrate_predictions.csv"
)

# Needed to compute descriptors for substrates NOT already in features.csv
...
TARGET_SUBSTRATES = {
    ...
    "Substrate_008": {
        ...
        "key_enzymes": [
            ...
            "17", "18", "20", "21", "22", "26", "27", "28", "29",
            "30", "31", "32", "33", "34", "38", "50", "55", "73",
            "87", "110", "123",
        ],
    },
...
# =====================================================================
# Load training data / fit preprocessing
# =====================================================================
...
# =====================================================================
# Feature retrieval for external substrates
# =====================================================================
...
def calculate_external_feature_row(name, smiles):
    ...
    if missing:
        raise ValueError(f"{name}: missing CATNIP features: {missing}")
...
# =====================================================================
# Precomputed predictions loading for internal substrates (8 & 34)
# =====================================================================

def load_precomputed_predictions(name, config):
    df = pd.read_csv(PRECOMPUTED_PREDICTIONS_CSV)
    ...
    if subset.empty:
        raise ValueError(f"No precomputed predictions found for substrate ID {target_id}")
    ...
    subset["Correct"] = subset["Enzyme_ID"].apply(lambda x: normalize_id(x) in reactive)
...
# =====================================================================
# Nearest-neighbor / candidate generation / scoring for external
# =====================================================================
...
def get_candidates(neighbor_ids, neighbor_distances, neighbor_pca,
                    external_pca, interactions, as_table):
    ...
    as_table = as_table.copy()
    as_table["_node1_id"] = as_table["node 1"].apply(normalize_id)
    ...
    for neighbor_id, neighbor_distance, neighbor_pca_vector in zip(
        ...
        for enzyme_id in enzymes_found:
            similar = as_table[
                as_table["_node1_id"] == normalize_id(enzyme_id)
            ][["node 2", "AS %"]].sort_values("AS %", ascending=False).head(10)
    ...
    if not results:
        raise RuntimeError(
            f"No enzyme candidates generated from the {SUBSTRATE_NEIGHBORS} "
            "nearest substrates. Check reaction_table.csv / alignment_scores.csv IDs."
        )
...
# =====================================================================
# Predict one substrate end-to-end
# =====================================================================

def predict_substrate(name, config, data, train, train_pca, scaler, pca,
                       interactions, as_table, ranker):

    if config["type"] == "internal":
        return load_precomputed_predictions(name, config)
    ...
    candidates = get_candidates(
        neighbor_ids, neighbor_dist, neighbor_pca, external_pca, interactions, as_table
    )
...
# =====================================================================
# Custom formatting function for output matching request
# =====================================================================

def format_and_print_results(name, top10):
    top10["Ground_Truth"] = top10["Correct"].astype(int)
    top10["PredProb"] = top10["score"]
    
    if "Uncertainty" not in top10.columns:
        top10["Uncertainty"] = 0.1000

    print("\n" + "=" * 60)
    print(f" SUBSTRATE: {name} (Top 10 Predictions)")
    print("=" * 60)
    
    display_df = top10[["Rank", "Enzyme", "Ground_Truth", "PredProb", "Uncertainty"]].copy()
    display_df["PredProb"] = display_df["PredProb"].map(lambda x: f"{x:.4f}")
    display_df["Uncertainty"] = display_df["Uncertainty"].map(lambda x: f"{x:.4f}")
...
def main():
    ...
    as_table = pd.read_csv(ALIGNMENT_TABLE)
    as_table = as_table[as_table["AS %"].notna()].copy()
    as_table["AS %"] = as_table["AS %"].map(lambda x: float(str(x).rstrip("%")) / 100)
