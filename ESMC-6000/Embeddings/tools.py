...
def get_candiates(neighbor_substrate_ids, neighbor_distances, interactions, es_table, neighbor_pca, substrate_pca):
    results_to_rank = []
        ...
        for enzyme_id in enzymes_found:
            expanded_enzymes.append((
                ...
                es_table[es_table['node 1'] == enzyme_id][
                    ['node 2', 'ES %']
                ].sort_values(by='ES %', ascending=False)[:10]
            ))
...
def get_score(dataset, interactions, es_table, pca, scoring_formula, mode='training'):
    ...
    for row_id, row in substrates_with_interactions.iterrows():
        ...
        results_to_rank = get_candiates(neighbor_substrate_ids, neighbor_distances, interactions, es_table,
...
def get_formula_baseline():
    def formula(data):
        return - 0.1 * data['distance'] - 0.9 * (1 - data['ES %'] ** 2) ** 2
