...
from torch.utils.data import WeightedRandomSampler  # Make sure to import this at top of file
...
def load_data(datadir, drug, target, batch_size, target_max_len=550):
    ...
    # ================= ADDED SAMPLER HERE =================
    # Compute inverse class frequencies for the training split
    class_counts = train[labelcol].value_counts()
    class_weights = {0: 1.0 / class_counts[0], 1: 1.0 / class_counts[1]}
    sample_weights = train[labelcol].map(class_weights).values

    sampler = WeightedRandomSampler(
        weights=torch.tensor(sample_weights, dtype=torch.double),
        num_samples=len(sample_weights),
        replacement=True  # Allows positive samples to be repeatedly drawn
    )
    # ======================================================

    # Pass 'sampler=sampler' instead of 'shuffle=True'
    train_generator = DataLoader(
        train_dataset, 
        batch_size=batch_size, 
        sampler=sampler,  # <-- Added sampler (shuffle removed)
...
def evaluate(model, dataloader, verbose: bool, device, savefolder=None, threshold: float = 0.5):   
    """
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            pred = (prob >= threshold).long()      # predicted class index based on threshold
    ...
    # After concat accumulated lists:
    prob_arr = torch.cat(prob_list).numpy()
    pred_arr = torch.cat(pred_list).numpy()
    gold_arr = torch.cat(gold_list).numpy()
    d_q_arr = torch.cat(d_q_list).numpy()
    ...
    k = 10
    # Sort descending by predicted probability of class 1
    sorted_indices = np.argsort(prob_arr)[::-1]
    top_k_indices = sorted_indices[:k]
    
    gold_top_k = gold_arr[top_k_indices]
    
    # Precision@k: fraction of top-k items that are relevant (gold == 1)
    precision_at_k = np.sum(gold_top_k) / k if k > 0 else 0.0
    
    # Recall@k: fraction of total relevant items captured in top-k
    ...
    # Enrichment@k: (Precision@k) / (Global Positive Rate)
...
if __name__ == "__main__":
    ...
    # 1. Sweep thresholds on VALIDATION data to find optimal cutoff
    logger.info('=' * 5 + ' Finding Best Threshold on Validation Set ' + '=' * 5)
    model.eval()
    val_probs, val_golds = [], []
    with torch.no_grad():
        for dr, tr, msk, label, _, _ in val_generator:
            dr, tr, msk = dr.to(device), tr.to(device), msk.to(device)
            log_probs, _, _, _, _, _ = model.predict(dr, tr, msk)
            val_probs.extend(torch.exp(log_probs)[:, 1].cpu().numpy())
            val_golds.extend(label.numpy())

    val_probs = np.array(val_probs)
    val_golds = np.array(val_golds)

    best_thresh = 0.5
    best_val_mcc = -1.0
    for t in np.linspace(0.01, 0.99, 99):
        t_preds = (val_probs >= t).astype(int)
        score = metrics.matthews_corrcoef(val_golds, t_preds)
        if score > best_val_mcc:
            best_val_mcc = score
            best_thresh = t

    logger.info(f"Optimal Threshold: {best_thresh:.3f} | Best Val MCC: {best_val_mcc:.4f}")

    # 2. Evaluate on TEST set using the tuned threshold
    logger.info('=' * 10 + ' Testing ' + '=' * 10)
    test_metrics = evaluate(
        model, test_generator, 
        verbose=True, device=device, 
        savefolder=running_folder, 
        threshold=best_thresh  # <-- Pass tuned threshold here
    )
