...
def get_train_and_test():
    data = pd.read_csv('/raid/data/smunoz/catnip/data/features.csv')
...
def load_interactions():
    interactions = pd.read_csv('/raid/data/smunoz/catnip/data/reaction_table.csv')
    return interactions


def load_sequence_similarity():
    es_table = pd.read_csv('/raid/data/smunoz/catnip/ESMC-6000/embedding_similarity_table.csv')
    es_table = es_table[es_table['ES %'].notna()]
    return es_table


def load_alignment_similarity():
    as_table = pd.read_csv('/raid/data/smunoz/catnip/data/alignment_scores.csv')
...
def get_candiates(neighbor_substrate_ids, neighbor_distances, interactions, es_table, as_table, neighbor_pca, substrate_pca):
    results_to_rank = []
    ...
    for neighbor_substrate_id, neighbor_distance, neighbor_pca_ in zip(neighbor_substrate_ids, neighbor_distances,
        ...
        for enzyme_id in enzymes_found:
            ...
            es_candidates = (
                es_table[es_table['node 1'] == enzyme_id]
                [['node 2', 'ES %']]
                .sort_values(by='ES %', ascending=False)
                .head(10)
            )

            # AS candidates: top 10
            as_candidates = (
                as_table[as_table['node 1'] == enzyme_id]
                [['node 2', 'AS %']]
                .sort_values(by='AS %', ascending=False)
                .head(10)
            )

            # Union of candidates from BOTH methods
            candidates = es_candidates.merge(
                as_candidates,
                on='node 2',
                how='outer'
            )

            # Add substrate-neighbor information
            candidates['distance'] = neighbor_distance

            for i in range(neighbor_pca.shape[1]):
                candidates[f'neighbor_pca_{i}'] = np.linalg.norm(
                    neighbor_pca_[i] - substrate_pca[i]
                )

            results_to_rank.append(candidates)

    results_to_rank = pd.concat(
        results_to_rank,
        ignore_index=True
    )

    # Candidates missing from one similarity table get 0
    results_to_rank['ES %'] = results_to_rank['ES %'].fillna(0)
    results_to_rank['AS %'] = results_to_rank['AS %'].fillna(0)
...
def get_score(dataset, interactions, es_table, as_table, pca, scoring_formula, mode='training'):
    ...
    for row_id, row in substrates_with_interactions.iterrows():
        ...
        results_to_rank = get_candiates(neighbor_substrate_ids, neighbor_distances, interactions, es_table, as_table,
...
def get_formula_baseline():
    def formula(data):
            es_score = (1 - data['ES %'] ** 2) ** 2
            as_score = (1 - data['AS %'] ** 2) ** 2
